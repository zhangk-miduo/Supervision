...
public class MessageService {
    ...
    private final SecretCipher cipher; private final StringRedisTemplate redis; private final RestTemplate http; private final ObjectMapper json; private final WecomIntegrationService wecom; private final RobotAppService robotService;
    public MessageService(MessageDeliveryMapper d,WecomWebhookMapper h,WecomGroupMapper g,SecretCipher c,StringRedisTemplate r,RestTemplate t,ObjectMapper j,WecomIntegrationService w,RobotAppService robotService){deliveries=d;hooks=h;groups=g;cipher=c;redis=r;http=t;json=j;wecom=w;this.robotService=robotService;}
    ...
    public List<MessageDelivery>test(Map<String,Object>req,DataScope scope){req=new HashMap<>(req);req.put("callerAccountId",scope.accountId());req.put("idempotencyKey","test-"+UUID.randomUUID());return sendAll(req);}
    ...
    public MessageDelivery send(Map<String,Object>req){
        ...
        try{
            JsonNode response;
            if("GROUP_WEBHOOK".equals(d.getChannel())){WecomWebhook h=robotService.requireUsable(targetId,longValue(req.get("callerAccountId")));rateLimit(targetId);response=post(cipher.decrypt(h.getWebhookCipher()),payload(req));}
    ...
    public List<MessageDelivery>list(DataScope scope){LambdaQueryWrapper<MessageDelivery>w=new LambdaQueryWrapper<>();if(!scope.admin())w.inSql(MessageDelivery::getExecutionId,"SELECT id FROM supervision_task_execution WHERE owner_account_id="+scope.accountId());return deliveries.selectList(w.orderByDesc(MessageDelivery::getId).last("LIMIT 200"));}
    ...
    private String normalizeCode(String raw){if(raw==null)return"UNKNOWN";String x=raw.toLowerCase();if(x.contains("robot_share_revoked"))return"ROBOT_SHARE_REVOKED";if(x.contains("robot_disabled"))return"ROBOT_DISABLED";if(x.contains("rate")||x.contains("45009"))return"RATE_LIMITED";if(x.contains("target_unavailable")||x.contains("webhook"))return"TARGET_UNAVAILABLE";if(x.contains("timeout"))return"NETWORK_TIMEOUT";if(x.contains("40014")||x.contains("42001")||x.contains("permission"))return"AUTH_OR_PERMISSION";if(x.contains("unsupported")||x.contains("invalid"))return"CONTENT_INVALID";return"UNKNOWN";}
    private String readable(String code){return switch(code){case"ROBOT_SHARE_REVOKED"->"机器人已收回共享，未继续发送";case"ROBOT_DISABLED"->"机器人已停用，未继续发送";case"RATE_LIMITED"->"企微发送频率受限，请稍后重试";case"TARGET_UNAVAILABLE"->"目标群已停用或消息推送配置无效";case"NETWORK_TIMEOUT"->"连接企微超时，请稍后重试";case"AUTH_OR_PERMISSION"->"企微凭据失效或应用权限不足";case"CONTENT_INVALID"->"消息内容格式不受支持";default->"推送失败，请查看技术详情或联系管理员";};}
