...
<script setup lang="ts">import{computed}from'vue';import{useRoute,useRouter}from'vue-router';import{logout}from'@/api';import{clearSession,currentUser as user}from'@/auth/session';const route=useRoute(),router=useRouter();const authPage=computed(()=>route.path==='/login'||route.path==='/change-password');const isAdmin=computed(()=>user.value?.roles?.includes('ADMIN'));async function signOut(){try{await logout()}finally{clearSession();router.replace('/login')}}</script>
